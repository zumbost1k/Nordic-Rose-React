...
const SectionPhoto = () => {
    ...
    if (!isLoading.process) {
        const perPageChange = (value) => {
            ...
            const newPerPage = data.post ? Math.ceil(data.post.length / value) : 1;
            if (currentPageNumber > newPerPage) {
                setCurrentPageNumber(newPerPage);
            }
        }
        const getData = (currentPageNumber, pageSize) => {
            return data.post ? data.post.slice((currentPageNumber - 1) * pageSize, currentPageNumber * pageSize) : 1;
        };
